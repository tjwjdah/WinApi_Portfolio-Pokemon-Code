...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.lpszMenuName = nullptr;// MAKEINTRESOURCEW(IDC_WINAPIPROJECT);
    ...
    case WM_COMMAND:
        {
            ...
            case ID_CREATETILE:
                // modeless
            {
               DialogBox(hInst, MAKEINTRESOURCE(IDD_CTC), hWnd, CreateTileProc);               
            }
                break;
            case IDM_SAVE:
                SaveTile();
...
// 주말 과제
// PanelUI 설계
//  - ChildUI 로 타일 이미지 버튼 추가
//  - 해당 이미지를 클릭 한 후, 타일 셋팅하는 기능이 동작 할 것
//  - 페이지 전환 버튼을 ChildUI 로 추가할 것